//Archival files for VIEWS WITH PIVOT LOGIC
...
object ArchivalViews {

  def main(args: Array[String]) {

    // create Spark context with Spark configuration
    val spark = SparkSession.builder.appName("CMSArchivalViews").enableHiveSupport().getOrCreate()
    ...
    val inputDF = spark.sqlContext.sql(s""" SELECT CD.MEDIA_ID, CM.NAME AS EPISODE, CM.REF_SERIES_TITLE AS SHOW, CM.CONTENT_TYPE, CM.CONTENT_FILE_NAME, CM.SBU, CM.CONTENT_DURATION, CM.TELECAST_DATE, CM.START_DATE, CM.GENRE, REGEXP_REPLACE(CM.CONTENT_SYNOPSIS, '\"', '') CONTENT_SYNOPSIS, DATEDIFF(CURRENT_DATE, DATE_FORMAT(FROM_UNIXTIME(UNIX_TIMESTAMP(CM.START_DATE, 'yyyy-MMM-dd HH:mm:ss')) ,'yyyy-MM-dd')) TENURE, CM.LANGUAGE, CD.DATE_STAMP, SUM(CD.NUM_VIEWS) VIEWS FROM CMS_ARCHIVAL CD LEFT JOIN CONTENT_MAPPER CM ON CD.MEDIA_ID = CM.ID WHERE CD.DATE_STAMP >= '$startDate' AND CD.DATE_STAMP <= '$endDate' GROUP BY CD.MEDIA_ID, CM.NAME, CM.REF_SERIES_TITLE, CM.CONTENT_TYPE, CM.CONTENT_FILE_NAME, CM.SBU, CM.CONTENT_DURATION, CM.TELECAST_DATE, CM.START_DATE, CM.GENRE, CM.CONTENT_SYNOPSIS, CM.LANGUAGE, CD.DATE_STAMP """)
    //val inputDF = spark.sqlContext.sql("SELECT CD.MEDIA_ID, CM.NAME AS EPISODE, CM.REF_SERIES_TITLE AS SHOW, CM.CONTENT_TYPE, CM.CONTENT_FILE_NAME, CM.SBU, CM.CONTENT_DURATION, CM.TELECAST_DATE, CM.START_DATE, CM.GENRE, REGEXP_REPLACE(CM.CONTENT_SYNOPSIS, '\"', '') CONTENT_SYNOPSIS, DATEDIFF(CURRENT_DATE, DATE_FORMAT(FROM_UNIXTIME(UNIX_TIMESTAMP(CM.START_DATE, 'yyyy-MMM-dd HH:mm:ss')) ,'yyyy-MM-dd')) TENURE, CM.LANGUAGE, CD.DATE_STAMP, SUM(CD.NUM_VIEWS) VIEWS FROM CMS_ARCHIVAL CD LEFT JOIN CONTENT_MAPPER CM ON CD.MEDIA_ID = CM.ID GROUP BY CD.MEDIA_ID, CM.NAME, CM.REF_SERIES_TITLE, CM.CONTENT_TYPE, CM.CONTENT_FILE_NAME, CM.SBU, CM.CONTENT_DURATION, CM.TELECAST_DATE, CM.START_DATE, CM.GENRE, CM.CONTENT_SYNOPSIS, CM.LANGUAGE, CD.DATE_STAMP")

    // apply logic OF PIVOT
    val pivotDF = inputDF.groupBy("MEDIA_ID", "EPISODE", "SHOW", "CONTENT_TYPE", "CONTENT_FILE_NAME", "SBU", "CONTENT_DURATION", "TELECAST_DATE", "START_DATE", "GENRE", "CONTENT_SYNOPSIS", "LANGUAGE", "TENURE").pivot("DATE_STAMP").sum("VIEWS").na.fill(0)
    ...
    joinedDF.coalesce(1).write.format("csv").mode("overwrite").option("header", "false").save(args(3))
