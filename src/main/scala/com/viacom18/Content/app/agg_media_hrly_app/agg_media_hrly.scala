...
object agg_media_hrly {
  ...
  def agg_media_hrly(date_zero: String , Load_type : String) = {
    ...
    sqlcontext.sql(s"""insert into DataTableloadHistory partition(date_stamp) select CURRENT_DATE ,'$date_zero' ,'F_Agg_Media_Hrly' ,'$Load_type','APP', '$count','$st_time','$end_time','$min','yes','content','$date_zero' as date_stamp from  DataTableloadHistory  limit 1""")
