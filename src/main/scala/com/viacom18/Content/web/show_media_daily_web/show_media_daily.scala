...
object show_media_daily {
  ...
  def show_media_daily(date_zero: String , Load_type : String) = {
    ...
    sqlcontext.sql(s"""insert into DataTableloadHistory partition(date_stamp) select CURRENT_DATE ,'$date_zero' ,'f_agg_media_dly' ,'$Load_type','WEB', '$count','$st_time','$end_time','$min','yes','content','$date_zero' as date_stamp from  DataTableloadHistory  limit 1""")
