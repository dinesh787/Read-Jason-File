...
object Ads_Viewer_all {
  val spark = SparkSession.builder().appName("Ads_Viewer_all").enableHiveSupport().getOrCreate()
  val sqlcontext: SQLContext = spark.sqlContext

  def main(args: Array[String]) {
    val LoadType = args(0)
    val Load_Data_date = args(1)
    val adsalesmapper = sqlcontext.sql("select * from Adsales_Mapper where table = 'voot_adsales_viewer_all' and flag=1")
  ...
  def Qry1(startdate: String, enddate: String, display_value: String)(implicit xc: ExecutionContext) = Future {
    ...
    try {
      ...
      sqlcontext.sql(s""" INSERT into TABLE voot_adsales_viewer_all partition(days_part_col,Qry_part_col)
      select  '$display_value','APP' Project,'All' Show, 'All' SBU,'All' language, count(distinct(distinct_id)),
     '$display_value' as days_part_col  , '01' as Qry_part_col
	  From voot_app_Adsales_base v Where v.event = ('mediaReady') and v.date_part_col >= '$startdate' and v.date_part_col <= '$enddate'
      """)
  ...
  def Qry2(startdate: String, enddate: String, display_value: String)(implicit xc: ExecutionContext) = Future {
    ...
    try {
      ...
      sqlcontext.sql(s""" INSERT into TABLE voot_adsales_viewer_all
    select  '$display_value','WEB' Project,'All' Show, 'All' SBU,'All' language,
    count(distinct(distinct_id)), '$display_value' as days_part_col  , '02' as Qry_part_col
  ...
  def Qry3(startdate: String, enddate: String, display_value: String)(implicit xc: ExecutionContext) = Future {
    ...
    SELECT
      ...
      'All' SBU_CLUSTER,
      'All' language,
      COUNT(DISTINCT (v.distinct_id )), '$display_value' as days_part_col  , '03' as Qry_part_col
  ...
  def Qry4(startdate: String, enddate: String, display_value: String)(implicit xc: ExecutionContext) = Future {
    ...
    SELECT
      ...
      From voot_web_Adsales_base v
     Where v.event = ('First Play') and
    v.date_part_col >= '$startdate' and v.date_part_col <= '$enddate' group by
    show
    """)
  ...
  def Qry5(startdate: String, enddate: String, display_value: String)(implicit xc: ExecutionContext) = Future {
    ...
    SELECT
      ...
      SBU_CLUSTER,
    'All' language,
      COUNT(DISTINCT (v.distinct_id )), '$display_value' as days_part_col  , '05' as Qry_part_col
      From voot_app_Adsales_base v
     Where v.event = ('mediaReady') and
    v.date_part_col >= '$startdate' and v.date_part_col <= '$enddate'
    group by
  ...
  def Qry7(startdate: String, enddate: String, display_value: String)(implicit xc: ExecutionContext) = Future {
    ...
    SELECT
      ...
      'All' SBU_CLUSTER,
      language,
      COUNT(DISTINCT (v.distinct_id )), '$display_value' as days_part_col  , '07' as Qry_part_col
      From voot_app_Adsales_base v
    Where v.event = ('mediaReady') and
    v.date_part_col >= '$startdate' and v.date_part_col <= '$enddate'
    group by
