...
object Ads_Content_Viewer_state
{
  val spark = SparkSession.builder().appName("Ads_Content_Viewer_state").enableHiveSupport().getOrCreate()
  val sqlcontext: SQLContext = spark.sqlContext

  def main(args: Array[String]) {
    val LoadType = args(0)
    val Load_Data_date = args(1)
    val adsalesmapper = sqlcontext.sql("select * from Adsales_Mapper where table = 'voot_adsales_content_viewer_state' and flag=1")
  ...
  def Qry1(startdate:String,enddate:String,display_value:String)(implicit xc: ExecutionContext) = Future {
    ...
    try {
      ...
      sqlcontext.sql( s""" INSERT  INTO TABLE voot_adsales_content_viewer_state  partition(days_part_col,Qry_part_col)
select '$display_value','APP' Project, State,'All' City, 'All' Show,  'All' Age,'All' Gender,
  ...
  count(distinct(case when v.event IN('App Launched','App Access') then v.distinct_id else '' end)),'$display_value' as days_part_col  , '01' as Qry_part_col
...
group by state  """ )
  ...
  def Qry2(startdate:String,enddate:String,display_value:String)(implicit xc: ExecutionContext) = Future {
    ...
    try {
      ...
      sqlcontext.sql( s""" INSERT  INTO TABLE voot_adsales_content_viewer_state  partition(days_part_col,Qry_part_col)
select '$display_value','WEB' Project,State,'All' City, 'All' Show,  'All' Age,'All' Gender,
  ...
  count(distinct(case when v.event IN ('Page Viewed') then v.distinct_id else '' end)),'$display_value' as days_part_col  , '02' as Qry_part_col
...
group by state  """ )
  ...
  def Qry3(startdate:String,enddate:String,display_value:String)(implicit xc: ExecutionContext) = Future {
    ...
    try {
      ...
      sqlcontext.sql( s""" INSERT  INTO TABLE voot_adsales_content_viewer_state  partition(days_part_col,Qry_part_col)
select   '$display_value','APP' Project, State,'All' City,'All' Show,
Age,
'All' Gender,
    count(distinct(case when v.event = ('mediaReady') then v.distinct_id else '' end)),
  sum((case when v.event = ('mediaReady') then 1 else 0 end)),
  count(distinct(case when v.event IN('App Launched','App Access') then v.distinct_id else '' end)),'$display_value' as days_part_col  , '03' as Qry_part_col
From voot_app_Adsales_base v
Where
v.date_part_col >= '$startdate' and v.date_part_col <= '$enddate'
group by age,state""" )
  ...
  def Qry4(startdate:String,enddate:String,display_value:String)(implicit xc: ExecutionContext) = Future {
    ...
    try {
      ...
      sqlcontext.sql( s""" INSERT  INTO TABLE voot_adsales_content_viewer_state  partition(days_part_col,Qry_part_col)
select '$display_value','WEB' Project,State,'All' City, 'All' Show,
  Age,
'Not Tagged' Gender,
    count(distinct(case when v.event = ('First Play') then v.distinct_id else '' end)),
  sum((case when v.event = ('First Play') then 1 else 0 end)),
  count(distinct(case when v.event IN ('Page Viewed') then v.distinct_id else '' end)),'$display_value' as days_part_col  , '04' as Qry_part_col
...
group by age,State """ )
  ...
  def Qry5(startdate:String,enddate:String,display_value:String)(implicit xc: ExecutionContext) = Future {
    ...
    try {
      ...
      sqlcontext.sql( s""" INSERT  INTO TABLE voot_adsales_content_viewer_state  partition(days_part_col,Qry_part_col)
select   '$display_value','APP' Project, State,'All' City, 'All' Show,  'All' Age,
 Gender,
    count(distinct(case when v.event = ('mediaReady') then v.distinct_id else '' end)),
  sum((case when v.event = ('mediaReady') then 1 else 0 end)),
  count(distinct(case when v.event IN('App Launched','App Access') then v.distinct_id else '' end)),'$display_value' as days_part_col  , '05' as Qry_part_col
From voot_app_Adsales_base v
Where
v.date_part_col >= '$startdate' and v.date_part_col <= '$enddate'
...
'Not Tagged' Gender,
    count(distinct(case when v.event = ('First Play') then v.distinct_id else '' end)),
  sum((case when v.event = ('First Play') then 1 else 0 end)),
  count(distinct(case when v.event IN ('Page Viewed') then v.distinct_id else '' end)),'$display_value' as days_part_col  , '06' as Qry_part_col
From voot_web_Adsales_base v
Where
v.date_part_col >= '$startdate' and v.date_part_col <= '$enddate'
group by state  """ )
...
select   '$display_value','APP' Project,State,'All' City, 'All' Show,
 ...
 Gender,
    count(distinct(case when v.event = ('mediaReady') then v.distinct_id else '' end)),
  sum((case when v.event = ('mediaReady') then 1 else 0 end)),
  count(distinct(case when v.event IN('App Launched','App Access') then v.distinct_id else '' end)),'$display_value' as days_part_col  , '07' as Qry_part_col
From voot_app_Adsales_base v
Where
v.date_part_col >= '$startdate' and v.date_part_col <= '$enddate'
...
'Not Tagged' Gender,
    count(distinct(case when v.event = ('First Play') then v.distinct_id else '' end)),
  sum((case when v.event = ('First Play') then 1 else 0 end)),
  count(distinct(case when v.event IN ('Page Viewed') then v.distinct_id else '' end)),'$display_value' as days_part_col  , '08' as Qry_part_col
From voot_web_Adsales_base v
Where
v.date_part_col >= '$startdate' and v.date_part_col <= '$enddate'
group by state,age """ )
