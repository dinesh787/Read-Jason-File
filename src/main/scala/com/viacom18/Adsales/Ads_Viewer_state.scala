...
object Ads_Viewer_state
{
  val spark = SparkSession.builder().appName("Ads_Viewer_state").enableHiveSupport().getOrCreate()
  val sqlcontext: SQLContext = spark.sqlContext

  def main(args: Array[String]) {
    val LoadType = args(0)
    val Load_Data_date = args(1)
    val adsalesmapper = sqlcontext.sql("select * from Adsales_Mapper where table = 'voot_adsales_viewer_region' and flag=1")
  ...
  def Qry1(startdate: String, enddate: String, display_value: String)(implicit xc: ExecutionContext) = Future {
    ...
    try {
      ...
      sqlcontext.sql(
        s""" INSERT into TABLE voot_adsales_viewer_region partition(days_part_col,Qry_part_col)
         select  '$display_value','APP' Project,'All' Show, 'All' SBU,'All' language, state,count(distinct(distinct_id)),
  ...
  def Qry2(startdate: String, enddate: String, display_value: String)(implicit xc: ExecutionContext) = Future {
    ...
    try {
      ...
      sqlcontext.sql(
        s""" INSERT into TABLE voot_adsales_viewer_region partition(days_part_col,Qry_part_col)
          select  '$display_value','WEB' Project,'All' Show, 'All' SBU,'All' language, state,count(distinct(distinct_id)),
      ...
      sqlcontext.sql(
        s""" INSERT into TABLE voot_adsales_viewer_region partition(days_part_col,Qry_part_col)
          ...
          'All' language,
            ...
            From voot_app_Adsales_base v
            Where v.event = ('mediaReady') and
          v.date_part_col >= '$startdate' and v.date_part_col <= '$enddate' group by show,state  """)
