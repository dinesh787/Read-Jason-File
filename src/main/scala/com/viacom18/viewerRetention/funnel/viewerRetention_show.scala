...
object ViewerRetentionshow {
  ...
  def main(args: Array[String]) {
    ...
    show(date_zero, outputfilepath)

  }

  def show(date_zero: String, outputfilepath: String) = {
    ...
    val showfunneldf = sqlcontext.sql(s" select distinct_id, media_id, event,date_stamp,cm.ref_series_title as show_name from voot_app_base tempbase INNER JOIN cmdf cm ON tempbase.media_id = cm.id where date_stamp >= '$date_zero' and date_stamp <= date_add('$date_zero',6) ")
    showfunneldf.createOrReplaceTempView("showfunneldf")
    //for one day on distinct and event = mediaReady creating a base table
    val baseshowdf = sqlcontext.sql(s" select distinct distinct_id,cm.ref_series_title as show_name from showfunneldf afsdf INNER JOIN cmdf cm ON afsdf.media_id = cm.id where date_stamp = '$date_zero' and event='mediaReady'")
    baseshowdf.createOrReplaceTempView("baseshowdf")
    //for seven day on distinct and event = mediaReady creating a base table
    val sevendayshowdf = sqlcontext.sql("select distinct afsdf.distinct_id, afsdf.date_stamp,afsdf.show_name from showfunneldf afsdf INNER JOIN baseshowdf bsdf ON afsdf.distinct_id = bsdf.distinct_id and afsdf.show_name = bsdf.show_name INNER JOIN cmdf cm ON afsdf.media_id = cm.id where afsdf.event='mediaReady' group by  afsdf.distinct_id, afsdf.date_stamp,afsdf.show_name ")
    ...
    val showfunnelperdf = sqlcontext.sql(s"select distinct_id,event,media_id, cm.ref_series_title as show_name ,cm.content_type as content_type, date_stamp, event,app_version,duration,duration_seconds from voot_app_base vab INNER JOIN cmdf cm ON vab.media_id = cm.id where date_stamp >= '$date_zero' and date_stamp <= date_add('$date_zero',6) group by distinct_id,media_id, cm.ref_series_title,date_stamp,cm.content_type, event,app_version,duration,duration_seconds")
    showfunnelperdf.createOrReplaceTempView("showfunnelperdf")

    val baseshowperdf = sqlcontext.sql(s"select distinct_id,cm.ref_series_title as show_name from showfunnelperdf afsdf INNER JOIN cmdf cm ON afsdf.media_id = cm.id where date_stamp = '$date_zero' group by distinct_id,cm.ref_series_title")
