package com.viacom.viewerRetention.funnel
...
object ViewerRetentionfe {
  ...
  def main(args: Array[String]) {
    ...
    fullepisode(date_zero, outputfilepath)

  }

  def fullepisode(date_zero: String, outputfilepath: String) = {
    ...
    val fullepisodefunneldf = sqlcontext.sql(s" select distinct_id, media_id, event,date_stamp,cm.ref_series_title as show_name from voot_app_base tempbase INNER JOIN cmdf cm ON tempbase.media_id = cm.id where date_stamp >= '$date_zero' and date_stamp <= date_add('$date_zero',6) and event = 'mediaReady' ")
    ...
    val basefullepisodedf = sqlcontext.sql(s" select distinct distinct_id,cm.ref_series_title as show_name from fullepisodefunneldf afsdf INNER JOIN cmdf cm ON afsdf.media_id = cm.id where date_stamp = '$date_zero' and event='mediaReady' and cm.content_type = 'Full Episode'")
    ...
    val sevendayfullepisodedf = sqlcontext.sql("select distinct afsdf.distinct_id, afsdf.date_stamp,afsdf.show_name from fullepisodefunneldf afsdf INNER JOIN basefullepisodedf bsdf ON afsdf.distinct_id = bsdf.distinct_id and afsdf.show_name = bsdf.show_name INNER JOIN cmdf cm ON afsdf.media_id = cm.id where afsdf.event='mediaReady' and cm.content_type='Full Episode' group by  afsdf.distinct_id, afsdf.date_stamp,afsdf.show_name ")
    ...
    val fullepisodefunnelperdf = sqlcontext.sql(s"select distinct_id,event,media_id, cm.ref_series_title as show_name ,cm.content_type as content_type, date_stamp, event,app_version,duration,duration_seconds from voot_app_base vab INNER JOIN cmdf cm ON vab.media_id = cm.id where date_stamp >= '$date_zero' and date_stamp <= date_add('$date_zero',6) group by distinct_id,media_id, cm.ref_series_title,date_stamp,cm.content_type, event,app_version,duration,duration_seconds")
    fullepisodefunnelperdf.createOrReplaceTempView("fullepisodefunnelperdf")

    val basefullepisodeperdf = sqlcontext.sql(s"select distinct_id,cm.ref_series_title as show_name from fullepisodefunnelperdf afsdf INNER JOIN cmdf cm ON afsdf.media_id = cm.id where date_stamp = '$date_zero' and cm.content_type = 'Full Episode' group by distinct_id,cm.ref_series_title")
