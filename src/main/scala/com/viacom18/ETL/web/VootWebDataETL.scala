...
/**
  ...
  *
  */
object VootWebDataETL {

  val spark = SparkSession.builder().appName("VootWebDataETL").enableHiveSupport().getOrCreate()
  ...
  def processVootAppData(inputfilename: String, outputfilepath: String,jobtype:String,dateset:String,loadtype:String)  = {
    ...
    //Rename columns adhereing to SQL naming conventions . Remove characters other than alpha numeric ,space,underscore_
    for (renameCol <- vootTransformDF.columns) {
    ...
    //merge media id
    if(hasColumn(vootTransformDF,"media_id") & hasColumn(vootTransformDF,"mediaid")) {
      vootTransformDF = vootTransformDF.withColumn("media_id", concat_ws(",", col("media_id"), col("mediaid"))).drop("mediaid")
      vootTransformDF = vootTransformDF.withColumn("media_id", col("media_id").cast("string"))
    }
    ...
    vootTransformDF.write.partitionBy("date_stamp").format("parquet").mode("append").option("spark.sql.parquet.compression.codec", "gzip").save(outputfilepath) //Write to Parquet Files.
      ...
      sqlcontext.sql(s"""insert into default.DataTableloadHistory partition(date_stamp) select CURRENT_DATE ,CURRENT_DATE ,'ETL' ,'$loadtype','WEB', '$count','$st_time','$end_time','$min','yes','$jobtype','$dateset' as date_stamp from DataTableloadHistory limit 1""")
  ...
  def main(args: Array[String]) {
    ...
    try {
      ...
      println("jobtype "+job_type)
      println("date set "+dateset)
      println("loadtype set "+loadtype)
      processVootAppData(inputJsonFile,outputParquetFilesDirectory,job_type,dateset,loadtype)
    } catch {
      case e: Exception => {
        println("VOOT_MOBILE_SPARK_ETL ERROR !!!   :")
        println(e)
